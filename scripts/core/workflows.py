...
WORKFLOW_TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "high_quality_screening": {
        "name": "检索优先级筛选",
        "description": "按元数据完整性、影响力线索和获取便利性排序，可选期刊过滤",
        "steps": [
            {
                "command": "search",
                "args": {
                ...
                "description": "按检索优先级排序，可选期刊过滤"
            }
        ],
        "variables": ["topic", "year_from", "journal"],
        "expected_output": "按检索优先级排序的论文列表（0-100，不代表学术质量）",
    ...
    "interdisciplinary_research": {
        "name": "跨学科研究",
        "description": "稳定公开源并发检索 + 去重 + 检索优先级排序",
        "steps": [
            {
                "command": "search",
                "args": {
        ...
        "variables": ["topic"],
        "expected_output": "跨数据源去重后的检索优先级列表",
