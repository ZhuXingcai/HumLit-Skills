...
格式化参考文献、导出 BibTeX/RIS/表格或导入题录。

## 排除条件

凭记忆补写作者、年份、卷期页码、DOI 或其他缺失字段。

## 前置条件

需要有效 session/project 或受支持的 NoteExpress、Refworks、BibTeX 文件。
...
返回 JSON 或显式请求的 raw 文本；缺失元数据保持可见，条件性补全失败不改变已有字段。

## 停止与降级

题录解析失败或无 session 时停止；CNKI/Crossref 不可用时仍格式化已有字段，但不得声称完整。
