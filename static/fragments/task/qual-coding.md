...
按显式编码簿对访谈/田野文本做关键词、正则命中和共现统计。

## 排除条件

把确定性命中称为语义开放编码、主题发现、理论饱和或因果分析。

## 前置条件

需要文本和合法 codebook；开放编码与主题解释需研究者阅读材料。
...
返回精确命中、定位、频次、未覆盖段落和段落级共现；零命中仍可是成功结果。

## 停止与降级

编码簿或正则非法时停止；未编码段较多时提示补充开放编码，不自动发明主题。
