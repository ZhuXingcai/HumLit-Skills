...
- 完成一次认证后，后续 CNKI 命令复用 `.humlit/browser-profile` 和 `.humlit/cookies.json`；同一项目/会话内通常无需反复登录，除非机构会话过期或用户使用 `auth-cnki --force`
...
- **沙盒环境自动脱困**：当沙盒内启动浏览器崩溃时（`0x80000003` 等），脚本自动在沙盒外启动浏览器（通过 `CREATE_BREAKAWAY_FROM_JOB` 或 `ShellExecuteW`），再用远程调试端口连接，用户无需任何手动操作。可通过 `HUMLIT_DEBUG_PORT` 环境变量自定义端口（默认 9222）

## 缓存

所有缓存在项目目录下 `.humlit/`（自动创建），建议加入 `.gitignore`。
API 缓存统一使用 `cache_ttl_days`，写入采用临时文件 + `fsync` + 原子替换。损坏 JSON 会重命名为 `*.corrupt-<timestamp>` 并只向 stderr 报告，不污染机器可读 stdout。

```
.humlit/
├── session.json        ← 搜索会话
├── openalex_*.json     ← API 缓存（统一按 cache_ttl_days，默认 30 天）
└── fulltext/           ← HTML 全文缓存
```
...
|------|---------------------------|-------------------|------------|
| OpenAlex / S2 / arXiv / NSSD / DBLP | 条件可用（需网络/上游正常） | 条件可用（需允许网络） | 条件可用（需允许网络） |
| 引用生成 / 导出 / cite / export | 安装依赖后可用 | 安装依赖后可用 | 有本地文件系统和 Python 时可用 |
| read-paper / write-docx / patch-docx / pdf-meta | 安装锁定依赖后可用 | 安装锁定依赖后可用 | 取决于文件系统与依赖 |
| 知网搜索 (search --source cnki) | 需校园网/VPN/CARSI + 浏览器 | 同左，且可能需授权 | 不可用 |
| 知网批量搜索 (batch-search) | 需校园网/VPN/CARSI + 浏览器 | 同左，且可能需授权 | 不可用 |
| 知网下载 (batch-download) | 需合法访问权限 + 浏览器 | 同左，且可能需授权 | 不可用 |
| 知网全文 (read-detail / detail) | 需合法访问权限 + 浏览器 | 同左，且可能需授权 | 不可用 |

Agent 判断依据：CNKI 读取 `capabilities.needs_escalation/cnki_feasible`；公开源的 live 可用性读取实际搜索返回的 `source_statuses`，不能用 `check.connector_available` 代替。
...
`check --fix` 会检测沙箱环境并返回 `recommended_actions`，但不会执行 `pip install`、写入 `.codex/config.toml`/`.claude/settings.json` 或替用户授权网络。Agent 读取 `capabilities` 判断能力，只有任务确需 CNKI 时才按建议和平台机制请求用户/管理员授权。
...
- 错误码 429 表示超过速率限制
- 命令会快速返回 `API_RATE_LIMIT`，不会在内部隐藏等待数分钟；可立即切换其他源，或按响应策略稍后重试
- 注册 API key 可提高配额，并通过 `SEMANTIC_SCHOLAR_API_KEY` 或 config.json 的 `semantic_scholar_api_key` 配置: https://www.semanticscholar.org/product/api#api-key
